...
COMMENT ON TABLE public.engagement_provider_queue IS
  'Ordered on-demand provider acceptance queue; only position 1 is primary (BOOKED).';
...
-- Backfill queue position 1 for existing assigned on-demand engagements
...
COMMENT ON TABLE public.engagement_provider_declines IS
  'Providers who explicitly declined an on-demand booking offer.';
